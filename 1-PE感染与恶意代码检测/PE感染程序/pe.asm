...
VCode:
    ...
    k32Base                        dd    ?  
    u32Base                        dd    ?
    szIS                        db    "Infected by 郭靖宜",0                                    ;for debug
    szMsg1                        db    "重定位完毕，并获得所有API地址！"

;写win32病毒的基本技术。编译器没有给我们的病毒的变量，标号进行定位，因此需要我们自己来定位。宿主程序的导入表中不一定有我们要用到的API，我们需要自己查找。一些病毒在这里获得时间来执行破坏模块",0                    ;for debug
    szMsg2                        db    "是否观看病毒感染过程？",0
    szMsg10                        db    "开始感染，by郭靖宜，UID：U202211883",0
